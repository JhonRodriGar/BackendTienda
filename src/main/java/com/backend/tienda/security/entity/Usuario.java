package com.backend.tienda.security.entity;
...
public class Usuario {
    ...
    /*
    @NotNull
    @ManyToMany(fetch = FetchType.EAGER) //Si no se pone esto el sistema muestra error (fail en el método do filter failed to lazily initialize a collection of role). Un usuario puede tener varios roles y un rol puede pertenecer a varios usuarios, para implementar lo anterior se crea una tabla intermedia que va a contenedor dos campos, uno va a ser el id del usuario y el otro el  id del rol para relacionar cada rol con cada usuario.
    @JoinTable(name = "usuario_rol", joinColumns = @JoinColumn(name = "usuario_id"), //usuario_rol es el nombre de la tabla, usuario_id va a ser la columna a que hace referencia la otra tabla hacia esta
    inverseJoinColumns = @JoinColumn(name = "rol_id"))
    private Set<Rol> roles = new HashSet<>(); //Conjunto de roles
    */
    public Usuario() {
