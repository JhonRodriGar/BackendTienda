package com.backend.tienda.security.controller;

import com.backend.tienda.security.dto.UsuarioRequest;
import com.backend.tienda.security.jwt.JwtUtil;
...
import com.backend.tienda.security.dto.LoginResponse;
import com.backend.tienda.security.dto.LoginRequest;
...
import com.backend.tienda.security.service.UsuarioService;
...
public class AuthController {
    ...
    public ResponseEntity<?> nuevo(@Valid @RequestBody UsuarioRequest usuarioRequest, BindingResult bindingResult){ //BindingResult es para validarlo
        ...
        roles.add(rolService.getByRolNombre(RolNombre.ROLE_CLIENTE).get()); //Trae el ROLE_CLIENTE de la BD
        ...
        if(usuarioRequest.getRoles().contains("vendedor"))
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_VENDEDOR).get());

        if(usuarioRequest.getRoles().contains("proveedor"))
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_PROVEEDOR).get());
